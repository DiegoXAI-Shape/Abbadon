...
def _cargar_csv(csv_path):
    ...
    # Soporta 2 clases (binario) o 3 clases (legacy)
    if iou_clases.shape[1] == 2:
        iou_clases.columns = ['Mascota', 'Fondo']
    else:
...
def _imprimir_tabla(datos_lista):
    ...
    metricas = []
    tiene_borde = all('Borde' in iou_df.columns for _, iou_df, _, _ in datos_lista)
    ...
    if tiene_borde:
        filas.append(('IoU Borde ⚠️', 'borde', '{:>'+str(col_width)+'.4f}'))
...
def comparar_entrenamientos(*archivos):
    ...
    # Detectar si algún CSV tiene clase Borde
    tiene_borde = all('Borde' in iou_df.columns for _, iou_df, _, _ in datos_lista)
    ...
    if tiene_borde:
        ...
        _plot_iou_clase(axes[1, 2], datos_lista, 'Fondo', '🌿 IoU Fondo')
